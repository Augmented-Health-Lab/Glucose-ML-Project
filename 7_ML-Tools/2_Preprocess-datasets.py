...
def process_files(project_df, extracted_glucose_file_path, project, minimum_coverage):
    ...
    - Loads raw glucose CSVs
    - Resamples to 5-minute intervals (can modify this parameter)
    - Interpolate small gaps (up to 15 minutes) & identifies days with enough coverage
    - Saves processed datasets to "Preprocessed_data/"
    - Generates a summary manifest file to "Preprocessed_data/"
    ...
    for i, row in project_df.iterrows():
        ...
        out_folder = Path("ML-Ready-Datasets") / dataset
...
def main():
    ...
    manifest_df.to_csv("ML-Ready-Datasets/preprocessing_manifest.csv", index=False)
